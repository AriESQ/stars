...
If you prefer new JSON-RPC 2.0 over WebSocket API then you can try my [tns-csi](https://github.com/fenio/tns-csi) driver instead.

You can use the democratic-csi documentation and achieve the same results, but the reason I created this 
guide is that the democratic-csi docs cover multiple complex combinations of various 
technologies. If you just want to have NFS/iSCSI over API, then the whole setup guide can be much simpler.

### Prerequisites
You're here because you want to connect a Kubernetes cluster to a TrueNAS Scale-based NAS, right?
So you need:
...
* Nodes section needs to be run on the nodes of your cluster, preferably Debian-based
* NAS section needs to be done on your NAS, and the assumption is that this is a TrueNAS Scale-based solution
...
### Nodes
All your nodes should be capable of using NFS/iSCSI shares. This means that some extra packages need to be installed 
...
The hash at the beginning of lines means you have to run them as root. So if you're on Ubuntu, just add `sudo` in front of these commands.
For Fedora/RedHat-based systems, something like this should also work:
...
## Let's start with creating an API key

![API](api.gif)

The key copied into the clipboard needs to be used in the YAML files below as `apiKey`.

Dataset creation for both NFS and iSCSI
...
Be aware that the above setups aren't hardened at all. This is just a basic setup created for testing purposes.
If you want to use it in a production environment, then you have to tweak it and make it more secure.

### K8S

Side note: Below is an example of how to install democratic-csi using helm with a values file.
I don't actually do it manually like this. In reality, I'm deploying democratic-csi using flux, which handles my [homelab](https://github.com/fenio/homelab).
To see the actual values files used to deploy democratic-csi on my cluster, take a look at my [repo](https://github.com/fenio/homelab/tree/main/cluster/apps/disk/democratic-csi).
...
Now a few words about YAML files used as values for helm charts. 
Since democratic-csi tries to be universal and they cover a bunch of different scenarios (which is great, BTW!), it means that to prepare a file to use as values 
for a helm chart, you have to combine at least two files from their repos. They explain which files and under which conditions. 
But if you want to use just NFS/iSCSI and just over API, then their documentation might be a bit confusing.
The files prepared by me below are based on already combined files, but for reference, you will also find commands to create skeleton files that you can adapt yourself.
But I want to make it clear: below you will find a simplified version of two files grabbed from the democratic-csi repo, merged together, with comments removed and adapted to my needs.
Most people will have to do more or less the same, but NOT all of them. So be aware that my files might not be something that you can copy and paste into your environment.</parameter>
...
driver:
  config:
    ...
    nfs:
      ...
      shareMapallGroup: ""
```
As already mentioned, the above is a simplified/streamlined version of the values file. 
Obviously, you have to change at least the paths and IP of the server.

Below are the commands to get the full file with comments.
...
Notice that since TrueNAS 25, the TrueNAS API no longer returns "SCALE" in the version string, which will stop the democratic-csi controller from starting up properly.
...
This will change the csi-driver image from the "latest" tag to the "next" tag.
...
Again, the above is a simplified/streamlined version of the values file. Below are the commands to get the file with comments.
You have to change the portal details, so at least its IP, group, and initiator group.
...
If you're experimenting with various settings, make sure to uninstall the helm release every time, as the above command will try to update the existing release if it already exists.

State after installation (depends on how many nodes you've got in your cluster):
...
Files that we can use to test the actual PVC:
